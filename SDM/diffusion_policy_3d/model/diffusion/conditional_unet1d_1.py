...
class ConditionalUnet1D1(nn.Module):
    def __init__(self, 
        input_dim,
        local_cond_dim=None,
        ...
        cls_pred_branch = nn.Sequential(
            nn.Conv1d(in_channels=2048, out_channels=1028, kernel_size=1, stride=1, padding=0),  
            nn.ReLU(),
            nn.Conv1d(in_channels=1028, out_channels=2, kernel_size=1, stride=1, padding=0),   
        )
    ...
    def forward(self, 
        ...
        if classify_mode:
            if gan_flag:
                x = self.cls_pred_branch(x).permute(0,2,1)
            return x
